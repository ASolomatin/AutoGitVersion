...
testInitial() {
    assertEquals "0.1.0-alpha.1" "$(version)"
...
testFeature() {
    ...
    assertEquals "0.1.0-alpha.5" "$(version)"
}
...
testMultipleMajorCommits() {
    ...
    assertEquals "1.0.0-alpha.2" "$(version)"
    git commit --allow-empty -m SimpleCommit2 > /dev/null
    assertEquals "1.0.0-alpha.3" "$(version)"
}
...
testHotfixOnStage() {
    ...
    assertEquals "0.1.1-beta" "$(version)"
}
